...
`deleted_at timestamptz NULL` on every synced table: `commitment`,
`commitment_amount`, `commitment_allocation`, `occurrence`, `income_source`,
`expected_income`, `goal`, `member`, `category`.
...
5. A monthly job hard-deletes tombstones older than 180 days.

## Consequences

**The constraint change that will bite if missed.** ADR-003's
`UNIQUE (commitment_id, due_date)` now collides with tombstones — a soft-deleted
occurrence blocks regenerating that same date. It must become partial:

```sql
CREATE UNIQUE INDEX uq_occurrence ON occurrence (commitment_id, due_date)
